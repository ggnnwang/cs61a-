...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if (isinstance(ant1, ContainerAnt) and ant1.can_contain(ant2)):
                ant1.contain_ant(ant2)

            elif (isinstance(ant2, ContainerAnt) and ant2.can_contain(ant1)):
                place.ant = ant2
                ant2.contain_ant(ant1)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while True:
            if self.max_range >= i >= self.min_range:
                if current_tile.bees:
                    # REPLACE THIS LINE
                    return rANTdom_else_none(current_tile.bees)
            current_tile = current_tile.entrance
            if current_tile == beehive:
                return
            i += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        def fire_damage(x, y):
            copy_place_bees = y.bees[:]
            for b in copy_place_bees:
                b.reduce_armor(x)

        temp_place = self.place
        temp_armor = self.armor

        fire_damage(amount, self.place)

        if temp_armor <= amount:
            fire_damage(self.damage, temp_place)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        which = rANTdom_else_none(self.place.bees)
        if not which:
            return
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while current_tile:
            if current_tile.ant and not current_tile.ant.hasbuffed:
                current_tile.ant.damage *= 2
                current_tile.ant.hasbuffed = True
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        temp_queen_armor = self.armor
        temp_queen_imposter = self.imposter
        super().reduce_armor(amount)
        temp_queen_armor -= amount
        if temp_queen_armor <= 0 and not temp_queen_imposter:
            bees_win()
...
def make_slow(action, bee):
    ...
    #make_slow is a status that takes an action method and a bee, 
    # and returns a new action method that performs the original action on every turn that gamestate.time is even and does nothing on other turns.
...
def apply_status(status, bee, length):
    """Apply a status(acction method) to a BEE that lasts for LENGTH turns."""
    # BEGIN Problem Optional 4
    "*** YOUR CODE HERE ***"
