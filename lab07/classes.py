...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for kard in player.deck.cards:
            kard.attack += other_card.attack
            kard.defense += other_card.defense
        
        copy = opponent.deck.cards[:]
        for kard in copy:
            if kard.attack == other_card.attack and kard.defense == other_card.defense:
                opponent.deck.cards.remove(kard)
    ...
    #adds the opponent card's attack and defense to all cards in the player's deck and then removes all cards in the opponent's deck that have the same attack or 
    # defense as the opponent's card.
