...
def has_cycle(link):
    ...
    seenlink = []
    oldlink = link
    while oldlink != Link.empty:
        seenlink.append(oldlink)
        oldlink = oldlink.rest
        if oldlink in seenlink:
            return True
    return False
...
s = Link(1, Link(2, Link(3)))
s.rest.rest.rest = s        # s = [1,2,3,1,2,3,1,2,3]
a = has_cycle_constant(s)
print(a) #>bool # True or False?
