...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operator_obj = env[self.operator.var_name] #got the primitive built-in func ; #PrimitiveFunction(operator.add)
        arguments_value = [a.eval(env) for a in self.operands]
        return operator_obj.apply(arguments_value)
...
# also, you can apply a function (an instance of PrimitiveFunction or LambdaFunction) by calling its apply method, which takes in a list of arguments (Value instances).
